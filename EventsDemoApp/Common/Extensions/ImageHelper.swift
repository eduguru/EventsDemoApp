...
public extension UIImage {
     ...
     */
    private func imageRef(withPadding padding: CGFloat, size: CGSize) -> CGImage {
        // Build a context that's the same dimensions as the new size
    ...
    func applyBlur(withRadius blurRadius: CGFloat, tintColor: UIColor?, saturationDeltaFactor: CGFloat, maskImage: UIImage? = nil) -> UIImage? {
        ...
        if hasBlur || hasSaturationChange {
            ...
            if hasBlur {
                ...
                let sqrtPi = CGFloat(sqrt(Double.pi * 2.0))
    ...
    /**
     ...
     - Returns A new image
     */
    class func image(fromURL url: String, placeholder: UIImage, shouldCacheImage: Bool = true, closure: @escaping (_ image: UIImage?) -> Void) -> UIImage? {
        // From Cache
        if shouldCacheImage {
        ...
        if let nsURL = URL(string: url) {
            session.dataTask(with: nsURL, completionHandler: { data, _, error in
                if error != nil {
